...
#     def is_active(self):
#         # Auto-deactivate after 30 sec
#         if self.active and (time.time() - self.activation_time) > 30:
#             self.deactivate()
#         return self.active
...
#         if not self.is_active(): return "🛑 Controller is inactive."
#         try:
#             change_volume(action)  # reliable volume change
#             await asyncio.sleep(0.2)
#             self.log(f"Volume control: {action}")
#             return f"🔊 Volume {action}."
#         except Exception as e:
...
# # ------------------------------
# # Live Real-Time Clock Feature
# # ------------------------------
# async def live_time_updater():
#     while True:
...
# # ------------------------------
# async def main():
#     asyncio.create_task(live_time_updater())
#     while True:
#         await asyncio.sleep(5)  # yahan aap test actions schedule kar sakte ho
